...
- **ステージ[2]〜[12]** — CSVパース → 検証 → 正規化 → 依存グラフ構築 → AOA変換 → スケジュール算出（ES/LS/クリティカルパス） → CCPMバッファ検出。**副作用を持たない純粋関数群。DOM・I/Oなし。** ファイルごとに独立して1回実行される（1ファイルの致命的エラーが他のファイルの処理を止めない）。単体テストの対象領域。**例外: 依存関係の手動編集（機能仕様書 4.3）を行うと、当該プロジェクトについてステージ[6]以降を編集後の依存辺で再実行する（[2]〜[5]は経由しない）。詳細設計書.md 10章。**
- **ステージ[13]〜[14]** — プロジェクト横断の集約、レイアウト、SVG描画（副作用あり、`workspace/`・`layout/`・`render/`・`ui/`）。

`workspace/pipeline.ts`（`processFile`）が1ファイル分のステージ2〜12を統括する（手動編集対応で「CSV→モデル」と「モデル→AOA」の2段に分割する予定。詳細設計書.md 10.1）。`workspace/Workspace.ts` は読み込み済み全ファイルを横断するステートフルなオーケストレータ（ファイルの追加・解除、ファイル間重複検出 E205/E206、共通時間軸、手動編集の適用）であり、`ui/` が唯一やり取りしてよいクラス。
...
| `ccpm/` | 合流バッファ候補の検出 |
| `edit/` | 依存関係の手動編集の純粋ロジック（辺の追加・削除、循環チェック、変更タスク差分、出力CSV文字列生成。機能仕様書 4.3 / 詳細設計書.md 10章） |
| `workspace/` | 複数ファイル・複数プロジェクトの状態管理、ファイル間重複排除、配色パレット、共通時間軸、手動編集の適用と再計算 |
| `layout/` | 複数プロジェクトを縦に積んだ座標計算 |
| `render/` | SVG生成 — テキストは必ず `textContent`、属性は必ず `setAttribute` のみで設定 |
| `ui/` | ファイル入力、ヘッダー・フッター、パネル、ズーム/パン、依存関係の手動編集ペイン。副作用を持つ唯一のエントリーポイント |
| `security/` | `dom.ts` — 詳細は下記 |

`csv/`〜`ccpm/` と `edit/` はDOMに依存しない純粋関数、`workspace/`・`layout/`・`render/`・`ui/` が副作用層。新しい純粋ロジックは対応するモジュールに置くこと（`ui/App.ts` に書かない）。
